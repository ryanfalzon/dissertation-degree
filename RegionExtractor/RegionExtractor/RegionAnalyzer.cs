...
namespace RegionExtractor
{
    class RegionAnalyzer
    {
                ...
                if(regions.Count != 0)
                {
                    ...
                    try
                    {
                        ...
                        if (regions.Count > 1)
                        {
                            ...
                            // Try and calculate the consensus sequence for the generated multiple sequence alignment
                            try
                            {
                                consensus = GetConsensus(alignedRegions);
                                msaLog.AppendLine($"{funfam.Funfam}, {consensus}");

                                // Try and remove the gaps from the front and back of the consensus sequence
                                try
                                {
                                    consensus = RemoveFrontBackGaps(consensus);
                                }
                                catch(Exception e)
                                {
                        ...
                        {
                            consensus = new string(regions.ElementAt(0).Select(a => (char)a).ToArray());
                        }

                        // Try and generate the kmers for the conserved region
                        try
                        {
                            FunctionalFamily currentFunFam = new FunctionalFamily(funfam.Funfam, consensus, GenerateKmers(consensus, 3));
                    ...
                    {
                        errorLog.AppendLine($"5, Error While Computing Multiple Sequence Alignment, Functional Family - {funfam.Funfam}");
                    }
                }
                else
                {
        ...
        private string GetConsensus(List<string> msa)
        {
            ...
            {
                return "No Data To Work Consensus Sequence On.";
            }
        ...
        private string RemoveFrontBackGaps(string consensus)
        {
            // Variables
        ...
        private List<string> GenerateKmers(string sequence, int kmerLength)
        {
            ...
            {
                temp = sequence.Substring(i, kmerLength);

                // Check if the kmer is made up of only gaps
                if (!temp.Equals("---"))
                {
                    kmers.Add(temp);
                }
        ...
        private void StoreData(List<StringBuilder> dataLogs, List<StringBuilder> statisticsLogs, List<StringBuilder> msaLogs, List<StringBuilder> errorLogs)
        {
            ...
            msaLog.AppendLine("Functional Family, Consensus Sequence, Threshold");
